...
DATABASE = os.environ.get('APP_DATABASE',os.path.join(os.path.split(__file__)[0], 'data', 'database.db'))
...
class OAuthRoutes:
    ...
    async def register_user(request: UserModels.RegisterRequest):
        ...
        try:
            res = []
            res.append(UserService.insert_user(request.username, request.full_name, formatted_email, hashed_password, user_uuid))
            return {"status": "success", "message": "User registered successfully"}            
        except sqlite3.IntegrityError:
            raise HTTPException(status_code=400, detail="Username already exists")

    @staticmethod
    @router.post(BASE_URL + "/edit")
    ...
    @router.get(BASE_URL+"/icon/{icon_name}", response_class=HTMLResponse)
